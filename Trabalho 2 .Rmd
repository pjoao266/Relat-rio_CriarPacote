...
<center>
```{r}
htmltools::img(src = knitr::image_uri("imagem10.png"),
               alt = 'currentData function')
```
</center>
...
<center>
```{r}
htmltools::img(src = knitr::image_uri("imagem11.png"),
               alt = 'print.tabelaBR function')
```
</center>
...
As documentações que você criou para as funções e bancos de dados, e a documentação do pacote precisam ser transformadas de forma que possibilite a instalação do pacote. As documentações de funções e bancos serão transformados em arquivos .Rd que criarão o help de cada função e banco. O que foi escrito no arquivo DOCUMENT será usado para atualizar o arquivo Namespace, que auxilia na instalação do pacote. Para executar isso é necessário rodar a função document do pacote devtools. 
...
O processo de mandar o pacote para o CRAN se torna bastante complicado pelo fato de ser muito criteriosa. A correção automática feita pelo CRAN retorna os errors (erros), warnings (avisos) e notes (notas). Notas são toleráveis, porém se for observado um erro ou aviso o pacote não será aceito. Após passar pela correção automática, o pacote será feita uma inspeção manual no pacote com objetivo de observar possíveis erros de escrita e não encaixe no padrão requerido pelo CRAN. Se o pacote não conter erros, ele será publicado, caso contrário você deve corrigir os erros apresentados e submeter o pacote para avaliação novamente. Segue abaixo um tutorial para a submissão.
